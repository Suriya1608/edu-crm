            ...
            <div class="wa-conv-list" id="convList">
                @forelse ($conversations as $conv)
                    ...
                    <a href="#"
                       ...
                       data-lead-url="{{ route('telecaller.leads.show', encrypt($conv->id)) }}">
                ...
                <div class="wa-chat-head" id="waChatHead">
                    ...
                    <div class="wa-chat-head-actions">
                        <a href="#" id="headLeadLink" class="btn btn-sm btn-outline-primary" target="_blank"
                           {{ $activeLead ? 'href="'.route('telecaller.leads.show', encrypt($activeLead->id)).'"' : '' }}>
...
(function () {
    ...
    const state = {
        ...
        leadUrl: @json($activeLead ? route('telecaller.leads.show', encrypt($activeLead->id)) : ''),
        lastMsgId: 0,
        pollTimer: null,
    };
    ...
    const SEND_URL_PATTERN  = @json(route('telecaller.leads.whatsapp.store', '__ID__'));
    const MEDIA_URL_PATTERN = @json(route('telecaller.leads.whatsapp.media', '__ID__'));
    const MSG_URL_PATTERN   = @json(route('telecaller.whatsapp.messages', '__ID__'));
